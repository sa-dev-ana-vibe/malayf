  ...
  return (
    <div className="p-[14px] flex flex-col gap-[12px]">
      <div className="text-[12px] text-muted-2 leading-[1.5]">
        Your master checklist, reused for every visit. Give each category a weight — they
        should add up to 100% and drive the overall score.
      </div>
      ...
      {categories.map((cat) => (
        ...
        >
          <div className="flex items-center gap-[6px] pl-[14px] pr-[10px] py-[9px] bg-soft border-b border-line-faint">
            <input
              value={cat.name}
          ...
          {cat.items.map((it) => (
            ...
            >
              <div className="flex items-center gap-[6px]">
                ...
                <input
                  value={it.text}
                  onChange={(e) => actions.renameItem(cat.id, it.id, e.target.value)}
                ...
                <select
                  value={itemType(it)}
                  onChange={(e) =>
                    actions.setItemType(cat.id, it.id, e.target.value as ItemType)
                  }
                  ...
                  {(it.options ?? []).map((op) => (
                    <div key={op.id} className="flex items-center gap-[5px]">
                      <input
                        value={op.label}
                        onChange={(e) =>
                          actions.updateOption(cat.id, it.id, op.id, "label", e.target.value)
                        }
                      ...
                      <input
                        value={op.value}
                        onChange={(e) =>
                          actions.updateOption(cat.id, it.id, op.id, "value", e.target.value)
                        }
                        inputMode="numeric"
                        placeholder="0"
                      ...
                      />
                      <button
                        onClick={() => actions.deleteOption(cat.id, it.id, op.id)}
      ...
      <div className="mt-[6px] border-t border-line pt-[14px]">
        ...
        <div className="text-[11px] text-faint leading-[1.5] mt-[8px]">
          ...
          текущие данные. «Добавить JSON» и «Вставить JSON» только дополнят список квартир из файла
          или буфера — чек-лист, метки и ред-флаги останутся как есть. «Скопировать промпт для LLM» даст
